/**********************************************************************
  TM.c:
     ...
     TM.c is a subroutine to calculate norm-conserving 
     pseudopotentials using TM's scheme.
...
{ 
  int m,n,l,i,i1,j,k,k1,k2,ip;
  int po,po1,numvps,ll,mul,mul1;
  int m1,n1,loop_num,loop_max;
  int num_nodes,so,mul2,m2,n2,loop_count;
  double max_SF,r,ef,rc1,diff,ep1,sr,tmp,tmp1,tmp2;
  double ep,rc,a2,sum,dnorm,da,dnorm_p;
  double a_p,lower_a,upper_a,lower_d,upper_d;
  double a11,a12,a13,a21,a22,a23,a31,a32,a33,det;
  double scaling,scaling_min,scaling_max;
  double W3[ASIZE1],rj[ASIZE1];
  double coe_sb[numsb+1],cmat[10][numsb+1];
  double c012[10][numsb+1],ia[10][10];
  double a0[ASIZE6][ASIZE6],c[ASIZE6];
	...
	for (mul=0; mul<NumVPS_L[l]; mul++){
	  ...
	  ep = E[state_num][so][n][l];
  	    ...
  	    po1 = Calc_VPS_PAO(0,so,l,rc,ep,1.0,PF[so][n][l],V,W2[so][m],V2[so][m]);
            ...
            /* calculate pseudopotential and pseudowave function by the Hamman method */ 
  ...
  for (so=0; so<SOI_switch; so++){
	...
	for (mul=1; mul<NumVPS_L[l]; mul++){
	  ...
	  ep1 = E[state_num][so][n1][l];
 	  ...
 	  for (i=0; i<Grid_Num; i++){
	    ...
	    cmat[2+mul2][numsb] = norm_ps - norm_all;
          ...
          /********************************************************
            express c0, c1, c2 by c3, c4, ...,c_(numsb-1) plus d
          ********************************************************/ 
	  ...
	  /* construct a matrix showing the linear relation between first few c0,c1,.. and others */
	    ...
	    /* calculate the Hessian matrix of the function F */

	    for (k1=(2+mul); k1<numsb; k1++){
	      for (k2=(2+mul); k2<numsb; k2++){
	        ...
	        for (i=0; i<(2+mul); i++){
		  tmp2 += c012[i][k1]*c012[i][k2]*Qcoes[i]; 
	        }

                sum = 4.0*tmp*tmp2;

                tmp1 = Lcoes[k1] + coe_sb[k1]*Qcoes[k1];
  	        for (i=0; i<(2+mul); i++){
		  tmp1 += c012[i][k1]*(Lcoes[i]+coe_sb[i]*Qcoes[i]); 
	        }

                tmp2 = Lcoes[k2] + coe_sb[k2]*Qcoes[k2];
  	        for (i=0; i<(2+mul); i++){
		  tmp2 += c012[i][k2]*(Lcoes[i]+coe_sb[i]*Qcoes[i]); 
	        }

                sum += 8.0*tmp1*tmp2;

                a0[k1-(2+mul)][k2-(2+mul)] = sum;
  	      }
	    }

            printf("Hessian mul=%2d\n",mul);

	    for (k1=(2+mul); k1<numsb; k1++){
	      for (k2=(2+mul); k2<numsb; k2++){
                printf("%10.5f ",a0[k1-(2+mul)][k2-(2+mul)]);
	      }
              printf("\n");
	    }
            ...
            for (k=0; k<(numsb-mul-2); k++){

	      for (i=0; i<(numsb-mul-1); i++){
		for (j=0; j<(numsb-mul-1); j++){
		  a1[i][j] = 0.0;
		}
	      }
		...
		for (j=0; j<(numsb-mul-1); j++){
		  a1[i][j] = a0[i][j];
  	      ...
  	      a1[k][numsb-mul-2] = 1.0;
  	      Gauss_LEQ(numsb-mul-3,a1,c);
  	      for (i=0; i<(numsb-mul-2); i++) ia1[i][k] = c[i];
	    }
	    ...
	    for (k=(2+mul); k<numsb; k++){
              ...
              sum = 0.0;
              for (i=0; i<(numsb-mul-2); i++){
                sum += ia1[k-(2+mul)][i]*gradF[i+(2+mul)];
              } 
              coe_sb[k] -= sum;
	    }
	  ...
	  for (k=0; k<(2+mul); k++){
	    ...
	    coe_sb[k] = sum;
          ...
          /* construct a pseudo-wave function which satisfies the generalized 
             norm conserving conditions */
