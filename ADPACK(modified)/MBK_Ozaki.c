...
{ 
  ...
  double ep,ep0,rc,a2,dif,sum,sum2,dnorm,da,dnorm_p,Chi0,Chi1;
	  ...
	  ep = E[state_num][so][n][l];
 	  ...
 	  for (i=0; i<Grid_Num; i++){
	    ...
	    r = MRV[i];
	      ...
	      else           Chi[so][m][i] = V2[so][m][i]*W2[so][m][i];
	    }
	...
	for (mul=1; mul<NumVPS_L[l]; mul++){
	  ...
	  ep1 = E[state_num][so][n1][l];
          ...
          if ( 1.0e-12<=OcpN[0][0][n1][l] ){
            ...
            printf("state_num=%2d so=%2d n1=%2d l=%2d mul=%2d RNG=%10.5f\n",
                    state_num,so,n1,l,mul,RNG[n1][l]);
  ...
  ************************************************************/
  ...
  /* ??? in input files, set energy of unbound states ??? */
