...
static void SCF_NL_Equation(int cs, double EZ, double dEeigen[ASIZE15],
                            double dE0[ASIZE15], double dE[ASIZE15]);
...
static double Func_dEdQ2(double Q);
static double Func_Eextra2(double Q);

static double Int_EDPP_Proj[ASIZE15];
static double Evps[ASIZE15][ASIZE13][ASIZE4];
static double CoesQ[ASIZE15+3];
static double CoesQ2[2*ASIZE15+3];
static double dEeigen[ASIZE15];

void Make_EDPP()
{
  static int cs,po,SCF,SCF_MAX;
  static int cs1,cs2,step,step_num;
  static double EZ,Step_EZ,Max_EZ,Q;
  static double scaleA[ASIZE15];
  static double Int_EDPP_Min[ASIZE15];
  static double dEeigen_Min[ASIZE15];
  static double EZ_Min[ASIZE15];
  static double dE[ASIZE15];
  static double dE0[ASIZE15];
  ...
  /*********************************************************
                 SCF calculations with EZ=0
  *********************************************************/

  EZ = 0.0;
  for (cs=0; cs<charge_state_num; cs++){
    SCF_NL_Equation(cs,EZ,dEeigen,dE0,dE);
    printf("cs=%2d dEeigen=%15.12f dE0=%15.12f\n",cs,dEeigen[cs],dE0[cs]);  
  }
  ...
  *********************************************************/
  ...
  for (cs1=0; cs1<charge_state_num; cs1++){
    Amat[cs1][charge_state_num+1] = dE0[cs1];
  }
  ...
  *********************************************************/
  ...
  for (cs=1; cs<charge_state_num; cs++){
    ...
    dEeigen_Min[cs] = 1.0e+10;

    /*  estimate a max EZ */

    Q = Int_EDPP_Proj[cs];
    Max_EZ = 0.03*fabs(Func_dEdQ(Q));
    step_num = 20;
    Step_EZ = 2.0*Max_EZ/(double)step_num;

    /*  search an appropriate EZ */

    for (step=0; step<step_num; step++){
      EZ = -Max_EZ + Step_EZ*(double)step;
      SCF_NL_Equation(cs,EZ,dEeigen,dE0,dE);
      printf("cs=%2d EZ=%15.12f dEeigen=%15.12f dE0=%15.12f\n",cs,EZ,dEeigen[cs],dE0[cs]);

      if (dEeigen[cs]<dEeigen_Min[cs]){
        dEeigen_Min[cs] = dEeigen[cs];
        EZ_Min[cs] = EZ;
        Eextra[cs] = dE0[cs];
        Int_EDPP_Min[cs] = Int_EDPP_Proj[cs];
      }
    }

    Int_EDPP_Proj[cs] = Int_EDPP_Min[cs];
  }

  for (cs=1; cs<charge_state_num; cs++){
    printf("cs=%2d EZ_Min=%15.12f dEeigen_Min=%15.12f\n",cs,EZ_Min[cs],dEeigen_Min[cs]); 
  }
  ...
  *********************************************************/
  ...
  for (cs1=0; cs1<charge_state_num; cs1++){
    for (cs2=0; cs2<2*charge_state_num; cs2++){
      Amat[cs1][cs2] = pow(Int_EDPP_Proj[cs1],(double)cs2);
    }
  }
          
  for (cs1=charge_state_num; cs1<(2*charge_state_num); cs1++){
    for (cs2=0; cs2<2*charge_state_num; cs2++){
      Amat[cs1][cs2] = (double)cs2*pow(Int_EDPP_Proj[cs1-charge_state_num],(double)(cs2-1.0));
    }
  }

  for (cs1=0; cs1<charge_state_num; cs1++){
    Amat[cs1][2*charge_state_num] = Eextra[cs1];
  }

  for (cs1=charge_state_num; cs1<2*charge_state_num; cs1++){
    Amat[cs1][2*charge_state_num] = EZ_Min[cs1-charge_state_num];
  }

  for (cs1=0; cs1<=2*charge_state_num; cs1++){
    for (cs2=0; cs2<=2*charge_state_num; cs2++){
      printf("%15.12f ",Amat[cs1][cs2]);
    }
    printf("\n");
  }  
  ...
  for (cs=0; cs<charge_state_num; cs++){
    ...
    printf("final cs=%2d EZ=%15.12f dE0=%15.12f dE=%15.12f\n",cs,EZ,dE0[cs],dE[cs]);
  }
...
void SCF_NL_Equation(int cs, double EZ, double dEeigen[ASIZE15], double dE0[ASIZE15], double dE[ASIZE15])
{
	...
	for (mul=0; mul<NumVPS_L[L]; mul++){
	  ...
	  rc = VPS_Rcut[m];
	    ...
	    MaxE = smallest(E[cs][so][n][L]+0.20,Vinf-1.0e-14);
  	    ...
  	    MinE = Evps[cs][so][m] - 0.10;
	    MaxE = smallest(Evps[cs][so][m]+0.10,Vinf-1.0e-14);
	  }
	  ...
	  ****************************************************/
	    ...
	    /*
            printf("SCF=%2d so=%2d L=%2d mul=%2d ene=%15.12f Trial_DD=%15.12f\n",
                    SCF,so,L,mul,ene,Trial_DD);
	    */
  ...
  for (so=0; so<SOI_switch; so++){
    for (L=0; L<ASIZE2; L++){
      for (mul=0; mul<NumVPS_L[L]; mul++){
        ...
        tmp = Evps[cs][so][m] - E[cs][so][n][L];
        dEeigen[cs] += OcpN[cs][so][n][L]*OcpN[cs][so][n][L]*tmp*tmp;
  ...
  ****************************************************/
  ...
  dE0[cs] = (Etot[cs]-Etot[0]) - (Etot0_VPS[cs]-Etot0_VPS[0]);
...
double Func_Eextra2(double Q)
{
  ...
  sum = 0.0;
  for (cs=0; cs<2*charge_state_num; cs++){
    sum += CoesQ2[cs]*pow(Q,(double)cs); 
  }

  return sum;
}

double Func_dEdQ2(double Q)
{
  static int cs;
  static double sum;
  ...
  for (cs=1; cs<2*charge_state_num; cs++){
    sum += (double)cs*CoesQ2[cs]*pow(Q,(double)(cs-1.0)); 
  }
